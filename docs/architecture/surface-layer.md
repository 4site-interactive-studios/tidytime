...
| **Capture** | `capture.browser`; `capture.heartbeat_seconds`; `capture.idle_threshold_seconds`; `capture.page_text_max_bytes`; **kill switches** per source: `capture.kill_switches.{app_watcher,chrome,calendar,fathom,slack}` (toggles); `sessionization.detour_tolerance_seconds`; `sessionization.min_session_seconds` |
